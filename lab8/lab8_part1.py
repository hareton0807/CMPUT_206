...
def  WaveletFusion(C1, C2):
   # input: C1, C2 = h by w matrix, wavelet transform of a image channel
    ...
    (height,width) = np.shape(C1)
    C = np.zeros((height,width),dtype='float32')
...
if __name__ == "__main__":
    ...
    for i in range(0,3):
        ...
        #------------------------------------------------------------        
                ...
                signal_A = fwt97_2d(im1_matrix[i],3)
                signal_B = fwt97_2d(im2_matrix[i],3)
                signal_A = np.asarray(signal_A)
                signal_B = np.asarray(signal_B)
                fusion = WaveletFusion(signal_A,signal_B)
                fusion = iwt97_2d(fusion,3)
                final_im_channels[:,:,i] = fusion        
